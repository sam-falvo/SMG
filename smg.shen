...
(define main
	...
	[InputFile | _] -> (verilog-module (load-smg-file InputFile) InputFile))
...
(define verilog-module
	[[module Name | MParams] | Body] InputFile ->
		(let Context (vector 20)
		     -       (process-module-decl Name MParams Context)
		     -       (output "// DO NOT EDIT~%")
		     -       (output "// This file is automatically generated!~%")
		     -       (output (make-string "// $ smg.shen ~A~%" InputFile))
		     -       (output "//~%// https://github.com/sam-falvo/smg~%~%")
		     ...
		     -       (output "endmodule~%")
		     true)
	X Y -> (error (make-string "Expected module clause; got:~%~S ~S" X Y)))
